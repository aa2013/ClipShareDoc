...
If you use WebDAV or S3 object storage as the forward method, you also need a notification service to notify clients about data changes. The notification content only contains the category and a unique data id. It does not include sensitive information, so it is safe to use.
