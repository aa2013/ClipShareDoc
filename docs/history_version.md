---
aside: "left"
---

## 📥 最新版本下载

| 平台      | 架构          | 格式  | 下载                                                                                                         | 版本    | 备注                      |
|---------|-------------|-----|------------------------------------------------------------------------------------------------------------|-------|-------------------------|
| Windows | x86_64      | exe | [下载](https://download.clipshare.coclyun.top/releases/clipshare/1.4.0/clipshare-1.4.0+22-windows-setup.exe) | 1.4.0 | 安装版，仅支持 Windows10+      |
| Windows | x86_64      | zip | [下载](https://download.clipshare.coclyun.top/releases/clipshare/1.4.0/clipshare-1.4.0+22-windows.zip)       | 1.4.0 | 便携版，仅支持 Windows10+      |
| Android | arm64-v8a   | apk | [下载](https://download.clipshare.coclyun.top/releases/clipshare/1.4.0/app-arm64-v8a-release-v1.4.0.apk)     | 1.4.0 | 要求`Android 7.0`及以上系统版本。 |
| Android | armeabi-v7a | apk | [下载](https://download.clipshare.coclyun.top/releases/clipshare/1.4.0/app-armeabi-v7a-release-v1.4.0.apk)   | 1.4.0 | 要求`Android 7.0`及以上系统版本。 |
| Android | x86_64      | apk | [下载](https://download.clipshare.coclyun.top/releases/clipshare/1.4.0/app-x86_64-release-v1.4.0.apk)        | 1.4.0 | 要求`Android 7.0`及以上系统版本。 |
| MacOS   | 通用架构        | dmg | [下载](https://download.clipshare.coclyun.top/releases/clipshare/1.4.0/clipshare-1.4.0+22-macos.dmg)         | 1.4.0 | 要求 `MacOS 10.15+`版本     |

> 更多历史版本请前往 [Github Releases](https://github.com/aa2013/ClipShare/releases) 下载。

## 📌 更新日志

### 🏷️ v1.4.0 - 2025-12-29

#### 通用

✨ 新增 Android端通知转发, 桌面端显示  
✨ 新增 文本分词  
✨ 新增 支持对设备连接过程中的参数加密  
✨ 新增 可配置同步数据范围  
🛠️ 修复 增加索引以解决当数据量多了后删除数据慢的问题  
⚡ 优化 增加设置以解决部分系统在锁屏状态下无法自动复制的问题  
⚡ 优化 更新后自动删除更新包文件  
⚡ 优化 设备发现流程，支持仅手动子网扫描和排除指定网卡  
⚡ 优化 标签管理显示排序  

#### Windows

🛠️ 修复 更新 Windows 托盘依赖，修复某些场景下的托盘消失问题  
🛠️ 修复 去除 Windows 安装器中的开机启动选项避免冲突导致最小化失效  

#### Android

✨ 新增 可配置熄屏后停止监听以节省电量  
✨ 新增 可在同步/新增数据后通过广播将数据共享给其他app
🛠️ 修复 部分系统在创建悬浮窗后系统底层崩溃导致软重启的问题  
🛠️ 修复 工作模式选择弹窗无权限时无法关闭的问题  
🛠️ 修复 隐藏后台无法保活的问题  
⚡ 优化 极大提高 Android 端后台保活能力  
⚡ 优化 日志支持生成当天的 Android 原生日志，增加手动清除按钮  

#### MacOS

\-

---

### 🏷️ v1.3.1 - 2025-11-24

#### 通用

🛠️ 修复 黑名单应用来源选择中系统应用列表错误地在用户应用中的问题  
🛠️ 修复 文件发送页面的弹窗关闭概率性导致主路由被弹出的问题  
🛠️ 修复 在安全认证页面取消导致后续无法弹窗的问题  
⚡ 优化 设备列表支持筛选在线状态并按设备名称排序  
⚡ 优化 即使跳过当前版本更新，在关于页中也可以手动检查弹窗  
⚡ 优化 修改更新弹窗中的文字避免被李跳跳自动点击  
⚡ 优化 黑名单增加类别条件

#### Windows

\-

#### Android

🛠️ 修复 root模式选择弹窗未自动关闭的问题  
⚡ 优化 用户引导完成页添加结束按钮到中央  
⚡ 优化 设备连接、配对通知自动取消，避免堆积到状态栏

#### MacOS

🛠️ 修复 MacOS上检测更新失败的问题

---

### 🏷️ v1.3.0 - 2025-11-09

#### 通用

✨ 新增 支持以Webdav/S3对象存储作为中转方式（实验性）  
✨ 新增 本地数据库查询编辑页面（关于页中）  
🛠️ 修复 有时弹窗无法正常关闭的问题  
🛠️ 修复 文件发送可能出现的路径解码错误导致实际成功但显示失败的问题  
🛠️ 修复 当 A->B,B->C 手动同步单条数据会丢失来源信息和标签信息的问题  
🛠️ 修复 在某些条件下因文件夹不存在导致的无法图片同步的问题  
⚡ 优化 允许按模块单独进行备份恢复处理  
⚡ 优化 删除数据时可能时间很久的问题  
⚡ 优化 中转配置支持扫码导入  
⚡ 优化 优化 中转服务状态增加【连接中】  
⚡ 优化 日志输出

#### Windows

🛠️ 修复 安装程序在安装时无法停止正在运行中的程序的问题

#### Android

🛠️ 修复 部分设备在初始化时因未取得存储权限导致的异常问题  
🛠️ 修复 可能导致闪退的异常的问题  
⚡ 优化 文件选择显示本地相册等选择器  
⚡ 优化 对新安装设备使用新的id获得方式避免同型号重复  
⚡ 优化 更新依赖，优化已安装app获取逻辑

#### MacOS

✨ MacOS 适配初始版本（2025-11-14）

---

### 🏷️ v1.2.1 - 2025-09-05

#### 通用

\-

#### Windows

🛠️ 修复 含默认值的快捷键清除后重启导致恢复为默认值的问题  
🛠️ 修复 记录窗体大小后重启会错误的问题

#### Android

\-

---

### 🏷️ v1.2.0 - 2025-09-01

#### 通用

✨ 新增 黑名单功能  
✨ 新增 备份和恢复功能  
⚡ 优化 一些其他不起眼的UI  
⚡ 优化 若启动初始化失败显示错误信息

#### Windows

🛠️ 修复 部分 Windows 设备启动获取设备信息失败的问题

#### Android

🛠️ 修复 悬浮窗中复制图片无法粘贴的问题

---

### 🏷️ v1.1.2 - 2025-07-25

#### 通用

✨ 新增 设备连接通知  
🛠️ 修复 设备信息同步报错的问题  
🛠️ 修复 剪贴板来源信息的可能不会同步的问题  
🛠️ 修复 导出Excel功能有可能导出数量不完整的问题  
🛠️ 修复 新增标签后搜索过滤器中不更新的问题  
⚡ 优化 允许在设备连接后不同步断连期间的数据，同时在设备操作弹窗中新增手动同步  
⚡ 优化 搜索/数据清理过滤器中的来源信息的筛选操作  
⚡ 优化 一些其他不起眼的UI

#### Windows

✨ 新增 桌面端新增显示主窗体和退出程序的快捷键设置  
🛠️ 修复 当自定义快捷键含有数字键时显示为 Unknown 的问题  
🛠️ 修复 在某些极少数特殊情况下，开启自启会无法自动连接中转的问题  
🛠️ 修复 桌面端若选择相同快捷键关闭弹窗，第二次会无法打开的问题  
⚡ 优化 桌面端对截图和接收的文件分别存储（Screenshots/files）

#### Android

🛠️ 修复 移动端若未选择将图片保存到相册，会保存至下载目录的问题  
🛠️ 修复 Android端双击记录无法复制图片的问题

---

### 🏷️ v1.1.1 - 2025-06-17

#### 通用

🛠️ 修复 剪贴板来源功能在某些情况下的空指针闪退问题

#### Windows

\-

#### Android

\-

---

### 🏷️ v1.1.0 - 2025-06-15

#### 通用

✨ 新增 剪贴板来源  
✨ 新增 设备断连通知选项

#### Windows

✨ 新增 Windows端应用锁  
🛠️ 修复 Windows端包含某些 emoji 会复制失败的问题

#### Android

⚡ 优化 Android 端启用应用锁后禁止截屏

---

### 🏷️ v1.0.0-beta 14 - 2025-06-09

#### 通用

⚡ 优化 仅在设备异常断开时弹出断连通知  
⚡ 优化 有更新时发起系统通知

#### Windows

\-

#### Android

🛠️ 修复 中转状态下亮起屏幕后连接断开的问题  
⚡ 优化 移动设备在数据网络下不扫描网络和广播发现

---

### 🏷️ v1.0.0-beta 13 - 2025-06-07

#### 通用

✨ 新增 导出 Excel 功能（搜索页面）  
✨ 新增 数据编辑功能，可修改历史数据（时间不会变化）  
✨ 新增 设备连接断开后通知提示  
🛠️ 修复 搜索过滤器的一些问题  
🛠️ 修复 标签管理中无法搜索的问题  
⚡ 优化 有更新时可以直接下载文件并打开  
⚡ 优化 清理数据时若该设备已无数据且未配对则自动移除设备信息  
⚡ 优化 优化一些 UI 显示

#### Windows

🛠️ 修复 Windows 端通过更新程序更新后重复快捷方式导致的开启启动无法最小化的问题  
🛠️ 修复 Windows 端通过托盘退出缓慢的问题

#### Android

🛠️ 修复 移动端从 WiFi 切换到数据网络后中转重连可能失败的问题（网络状态变化过快）

---

### 🏷️ v1.0.0-beta 12 - 2025-05-10

#### 通用

🛠️ 修复 主题切换不完全的问题  
🛠️ 修复 中转断开无法自动重连的问题  
⚡ 优化 文件传输历史记录可以全选  
⚡ 优化 设备连接逻辑  
⚡ 优化一些 UI 显示  
⚡ 优化一些操作逻辑

#### Windows

⚡ 优化 Windows弹窗允许以相同快捷键关闭

#### Android

✨ 新增 Android 剪贴板隐藏 api 监听模式，解决 OriginOS 等系统的监听问题  
🛠️ 修复 Android 端 Shizuku 服务进程不退出的问题  
⚡ 优化 Android 端允许在屏幕从关闭到亮起时不进行设备扫描

---

### 🏷️ v1.0.0-beta 11 - 2025-04-09

#### 通用

✨ 新增 桌面端历史弹窗搜索  
✨ 新增 文件拖拽同步  
⚡ 优化一些 UI 显示  
⚡ 优化一些操作逻辑

#### Windows

\-

#### Android

\-

---

### 🏷️ v1.0.0-beta 10 - 2025-03-09

#### 通用

✨ 新增 二维码扫码连接（手动添加时）  
✨ 新增 数据清理功能  
⚡ 优化一些 UI 显示

#### Windows

✨ 新增 Windows端历史记录弹窗记录上次位置

#### Android

✨ 新增 Android端支持截屏检测自动复制

---

### 🏷️ v1.0.0-beta 9 - 2025-01-19

#### 通用

⚡ 优化一些 UI 显示

#### Windows

🛠️ 修复 Windows端弹窗中未应用多语言翻译的问题  
🛠️ 修复 Windows端弹窗中无法预览图片的问题

#### Android

✨ 新增 Android端最近任务显示/隐藏选项

---

### 🏷️ v1.0.0-beta 8 - 2025-01-14

#### 通用

✨ 新增语言选项

#### Windows

\-

#### Android

🛠️ 修复 Android下同步图片后自动跳转app的问题

---

### 🏷️ v1.0.0-beta 7 - 2025-01-05

#### 通用

✨ 新增断连重连机制

#### Windows

\-

#### Android

\-
